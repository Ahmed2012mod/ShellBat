...
// IGraphicsEffectD2D1Interop is declared here and also in csproj as an alias otherwise it doesn't work
// also it requires CsWinRT to be referenced and CsWinRTAotOptimizerEnabled to be true (I think). what a mess.
// but what's nice is we don't need that poorly architectured Win2D crap
[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicProperties)]
public abstract partial class Effect(uint sourcesCount = 0) : IGraphicsEffect, IGraphicsEffectSource, IGraphicsEffectD2D1Interop
{
    ...
    public string Name { get; set; } = string.Empty; // *must* not be null for IGraphicsEffectD2D1Interop
    ...
    public Guid Clsid
    {
        get
        {
            var att = GetType().GetCustomAttribute<GuidAttribute>() ?? throw new InvalidOperationException("Type '" + GetType().FullName + "' must have a valid Guid attribute set.");
    ...
    private sealed class PropDef(PropertyInfo property, int index, GRAPHICS_EFFECT_PROPERTY_MAPPING mapping)
    {
        ...
        public HRESULT GetPropertyValue(Effect effect, out nint ptr)
        {
            ...
            ptr = 0;
            Application.TraceError(this + " " + this + " E_NOTIMPL");
    ...
    private static List<PropDef> GetPropDefs([DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicProperties)] Type type)
    {
        if (!_properties.TryGetValue(type, out var list))
        {
            ...
            {
                var att = prop.GetCustomAttribute<EffectPropertyAttribute>();
                if (att == null)
                    continue;

                var def = new PropDef(prop, att.Index, att.Mapping);
                list.Add(def);
            }
    ...
    HRESULT IGraphicsEffectD2D1Interop.GetNamedPropertyMapping(PWSTR name, out uint index, out GRAPHICS_EFFECT_PROPERTY_MAPPING mapping)
    {
        ...
        if (def == null)
        {
            index = 0;
            mapping = 0;
            Application.TraceError(this + " name:'" + name + "' E_INVALIDARG");
    ...
    HRESULT IGraphicsEffectD2D1Interop.GetProperty(uint index, out nint value)
    {
        try
        {
            ...
            {
                value = 0;
                Application.TraceError(this + " index:" + index + " E_BOUNDS");
                return DirectN.Constants.E_BOUNDS;
            }
        ...
        {
            Application.TraceError(this + " index:" + index + " E_FAIL");
            value = 0;
