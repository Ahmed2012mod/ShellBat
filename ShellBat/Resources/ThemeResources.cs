...
public static class ThemeResources
{
    ...
    private static void EnsureFiles(bool forceRefresh = false)
    {
        ...
        var startTok = typeof(ThemeResources).Namespace + @".Themes.";
        var files = asm.GetManifestResourceNames().Where(n => n.StartsWith(startTok)).Order().ToArray();
        var lastFile = files.Last();

        if (!forceRefresh)
        {
            var path = Path.Combine(Settings.ThemesPath, lastFile[startTok.Length..]);
            if (IOUtilities.FileGetSize(path) > 0)
            {
                //Application.TraceVerbose("Themes resources already exist, skipping extraction");
                _task = null;
                return;
            }
        }

        IOUtilities.DirectoryDelete(Path.GetDirectoryName(Settings.ThemesPath)!, true, false);
        foreach (var name in files)
        {
            using var stream = asm.GetManifestResourceStream(name) ?? throw new InvalidOperationException();
            var path = Path.Combine(Settings.ThemesPath, name[startTok.Length..]);
